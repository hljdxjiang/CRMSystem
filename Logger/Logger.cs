...
namespace JHC.Logger
{
    public class Logger : ILogger
    {
        ...
        void Write(LogLevel logLevel, EventId eventId, string message, Exception ex)
        {
            ...
            //TODO 提高效率 队列写！！！
            var log = String.Concat(DateTime.Now.ToCstTime().ToString("HH:mm:ss"), '[', logLevel.ToString(), ']', '[',
        ...
        bool CheckNeedCreateNewFile()
        {
            ...
            if (_FileName != DateTime.Now.ToCstTime().ToCstTime().ToString(this.FileNameTemplate))
            {
                return true;
            }
            return false;
